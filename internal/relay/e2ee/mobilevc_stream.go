...
func NewMobileVCStreamCodec(sessionID, clientID, handshakeID string, sendKey, receiveKey []byte, sendDir string) (*MobileVCStreamCodec, error) {
	...
	return &MobileVCStreamCodec{
		...
		SendDir: sendDir, ReceiveDir: receiveDir, seen: map[uint64]struct{}{},
	}, nil
}
...
func (c *MobileVCStreamCodec) Encode(messageID string, plaintext []byte) (RelayForwardFrame, error) {
	...
	ctx := c.frameContext(c.SendDir, counter)
	...
	return RelayForwardFrame{
		...
		StreamID: MobileVCStreamID, Counter: counter, HandshakeID: c.HandshakeID,
	}, nil
}
...
func (c *MobileVCStreamCodec) Decode(frame RelayForwardFrame) ([]byte, error) {
	...
	if _, ok := c.seen[frame.Counter]; ok {
	...
	plaintext, err := Decrypt(c.ReceiveKey, sealed, c.frameContext(c.ReceiveDir, frame.Counter))
	if err != nil {
		return nil, err
	}
	c.seen[frame.Counter] = struct{}{}
...
func (c *MobileVCStreamCodec) frameContext(direction string, counter uint64) FrameContext {
	return FrameContext{
		SessionID: c.SessionID, ClientID: c.ClientID, HandshakeID: c.HandshakeID,
		Direction: direction, StreamID: MobileVCStreamID, Counter: counter,
	}
}
...
func (c *MobileVCStreamCodec) validateFrame(frame RelayForwardFrame) error {
	...
	if frame.StreamID != MobileVCStreamID || frame.HandshakeID != c.HandshakeID {
