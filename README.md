...
- `@justprove/mobilevc` 已发布到 `0.2.10`，各平台预编译后端二进制包同步更新
- 修复了 `mobilevc public --relay` 成功连接一次后，后端停止再重启可能无法重新注册/配对的问题
- Android 安装包已切到 `arm64-v8a`，官网上的 Android 下载入口当前指向 `1.0.0 (20260601 arm64)`
- iOS Release / Profile 的 Archive 签名身份已明确为 `Apple Distribution`
- 移动端语音通话支持自动识别停顿并发送，同时延长静默判定，减少刚开口就被提前发送的问题
- 优化语音通话交给 Claude / Codex 后的执行中状态提示，避免执行中误提示“需要补充信息”
